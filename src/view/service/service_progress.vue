		...
		<div v-for="(item, index) in order_list">
			<div class="dd_product mt10">
				<div class="clearfix order-status">
					<strong>服务单号：{{item.serviceid}}</strong>
					<strong class="right color-ff2c2c">{{item.afsservicestepname}}</strong>
				</div>
				<div class="order-det">
					<div class="clearfix pro">
						<img :src="'http://img11.360buyimg.com/n1/'+item.skuimg" />
						<div class="ddpr_name">
							<div class="">
								<span class="font26 nowrap">{{item.skuname}}</span>
							</div>
							<div class="color-b0b0b0 font22">
								<span class="">订单编号：{{item.jdorderid}}</span>
							</div>
						</div>
					</div>
				</div>
				<div class="applay_btn clearfix">
					<router-link class="active" :to="{path:'/service/service_detail',query:{id:item.id}}">进度查询</router-link>
				</div>
			</div>
		</div>
		</mt-loadmore>
...
//				token:'eyJhbGciOiJIUzI1NiJ9.eyJ1dHV0dXQiOiIxNDMyNSMxNTY0MyMxMDMyIn0.OQ3MHInUv1HQgbfvRIKnv4DIi8ycHu5Ne7-0h5-uEHA',
		...
		methods:{
			getList(pagenum){
				let _this = this;
				_this.$tool.sendOPtion(this, com.order_hosturl + "/afterSale/schedulelist.json", {
					token:_this.token,
