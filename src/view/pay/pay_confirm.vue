<template>
	<div class="wrapper" v-title="'支付'">
		...
		<div class="confirm" v-if="paymentType==109">
			<div class="clearfix" @click="check0">
				<label :class="{active:isActive0}"></label>
				<span class="mr10">保障卡</span>
			</div>
		</div>
		<div class="confirm" v-if="(paymentType==107 ||paymentType==109)  && ordertype !=4">
			<div class="clearfix" @click="check1">
				<label :class="{active:isActive2}"></label>
				<span class=" mr10">福币</span>
				<!--<span class="color-ff2c2c left">{{deduction}}</span>-->
				<span class="right color-ff2c2c">{{aviCoin/100 | toFloat}}</span>
			</div>
		...
		<div class="confirm" v-if="paymentType==103 || paymentType==107 || paymentType==109">
			<div class="clearfix" @click="check2">
				...
				<span class="right color-ff2c2c" v-if="yueshow && isActive==true && isActive2==true">￥{{ (Math.round(order_totalprice*100) - aviCoin)/100 | places}}</span>
			</div>
		</div>
		<a class="pay" @click="pay">确认支付</a>
		<mt-popup v-model="popupVisible">
			<p>您尚未绑定保障卡,立即绑定？</p>
			<div>
				<a class="special" @click="sure()">是</a>
				<a @click="cancle()">否</a>
			</div>
		</mt-popup>
	</div>
...
//				token: 'eyJhbGciOiJIUzI1NiJ9.eyJ1dHV0dXQiOiIxNDMyNSMxNTY0MyMxMDMyIn0.OQ3MHInUv1HQgbfvRIKnv4DIi8ycHu5Ne7-0h5-uEHA',
		...
		mounted() {
			if(this.paymentType !=109 && this.paymentType !=103){
				this.getAvicion();
			}else if(this.paymentType ==103){
				this.isActive=true;
		...
		methods: {
			...
			check1() {
				if(parseFloat(this.aviCoin) == 0) {
					this.isActive2 = false;
				} else if(parseFloat(this.aviCoin / 100) < parseFloat(this.order_totalprice)) {
					this.isActive = true;
					this.isActive2 = true;
					this.yueshow = true;
			...
			check2() {
				if(parseFloat(this.aviCoin / 100) < parseFloat(this.order_totalprice)) {
					this.isActive = true;
					this.isActive2 = false;
					this.isActive0 = false;
				} else {
			...
			sure(){
				this.$router.push({
					path: '/wallet/selfSecurityCardBind/bindCard',
					query: {
					...
					if(res.success) {
						if(res.data == '' || res.data == null | res.data == "undefined") {
							...
							if(_this.aviCoin==0){
								_this.isActive2 = false;
								_this.isActive = true;
							}else if(parseFloat(_this.aviCoin / 100) < parseFloat(_this.order_totalprice)) {
								_this.isActive = true;
			...
			pay() {
				if(this.isActive0==true){
					this.payBzk()
				}else if(this.isActive2 && this.isActive == false) {
					this.payFb()
			...
			payFb() {
				let _this = this;
				if(_this.userCode != "") {
					_this.$tool.sendOPtion(this, com.order_hosturl + "/wechat/fbpay.json", {
						orderId: _this.orderId,
